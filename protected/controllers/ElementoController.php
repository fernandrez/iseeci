	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Elemento']))
		{
            ...
            if($model->save())
            {
            	$parent=$model->parent;
				if($parent){
					$dirImagen='images/'.$parent->titulo;
					if(!is_dir($dirImagen))
						mkdir($dirImagen);
					$imgName=@$_FILES['Elemento']['name']['imagen'];
					$dirImagen=$dirImagen.'/'.$model->titulo.'_'.md5($model->titulo.$model->id.mktime()).'.'.(end(explode('.',$imgName)));
				}
				else {
					$dirImagen='images/'.$model->titulo;
					$dirImagen=$dirImagen.'/'.$model->titulo.'_'.md5($model->titulo.$model->id.mktime()).'.'.(end(explode('.',$imgName)));
				}
				if(isset($_POST['Elemento']['imagen']) &&
				$_POST['Elemento']['imagen']!="")
					$model->imagen->saveAs($dirImagen);
				$model->dir_imagen=$dirImagen;
				$model->save();
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['Elemento']))
		{
			...
			if($model->save()){
				...
				var_dump($model->imagen->saveAs($dirImagen));die;
					$model->dir_imagen=$dirImagen;
				}
				$model->dir_imagen=$dirImagen;
				$model->save();
